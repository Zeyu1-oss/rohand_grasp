...
class CombinedScene:
    ...
    def add_lego_object(self, mesh_root_or_file: str,
                        friction=(2.0,0.01,0.001), density: float=1000.0,
...
def eval_one_lego(cfg: Dict[str,Any], lego_name_raw: str, show=False, debug=False):
    ...
    move_only_if_csv = bool(cfg.get("tighten_only_if_csv", True))
    ...
    fric_cfg  = cfg.get("friction", [2.0, 0.01, 0.001])
    ...
    def probe_until_both_ready() -> Dict[str, Any]:
        ...
        dp_base = abs(probe_total_cmd) / max(1, probe_steps)
        dp_floor = float(cfg.get("probe_step_floor", 0.15 * dp_base))
        # —— 接近距离门限（从松到紧）
        near_d3 = float(cfg.get("probe_near_d3", 5.0e-4))
        near_d2 = float(cfg.get("probe_near_d2", 2.0e-4))
        near_d1 = float(cfg.get("probe_near_d1", 1.0e-4))
        # —— 分段缩放
        scale_far   = float(cfg.get("probe_scale_far",   1.00))   # 远
        scale_mid   = float(cfg.get("probe_scale_mid",   0.50))   # 中
        scale_near  = float(cfg.get("probe_scale_near",  0.25))   # 近
        scale_touch = float(cfg.get("probe_scale_touch", 0.10))   # 轻触/微力
        # —— 微力阈值
        micro_force = float(cfg.get("probe_micro_force", max(1e-6, 0.25*precontact_force_eps)))

        # 单步仿真次数
        probe_sim_per_step = int(cfg.get("probe_sim_per_step", max(1, sim_per_step)))

        ready_if = False
        ready_th = False
        step_reached = 0

        def _advance_prox(prox_name: str, sign: float, step_cmd: float):
            curp = _read_joint(model, data, prox_name)
            newp = _set_joint_qpos_clamped(model, data, prox_name, curp + sign*step_cmd)
        ...
        for s in range(min(precontact_max_steps, probe_steps)):
            ...
            d_min = min(float(d_if), float(d_th))
            step_scale = scale_far
            if (fn_if >= micro_force) or (fn_th >= micro_force):
                step_scale = scale_touch
            elif d_min <= near_d1:
                step_scale = scale_near
            elif d_min <= near_d2:
                step_scale = scale_mid
            elif d_min <= near_d3:
                step_scale = 0.75*scale_far

            dp = max(dp_floor, dp_base * step_scale)

            if (not ready_if):
                _advance_prox(if_prox_name, -1.0, dp)
            if (not ready_th):
                _advance_prox(th_prox_name, -1.0, dp)

            _step_with_root_lock(probe_sim_per_step)
    ...
    def tighten_by_actuators_freeze_then_together() -> Dict[str, Any]:
        ...
        joint2aid = {}
        for nm in list(set(tighten_joints + [if_dist_name, th_dist_name])):  # CSV 需要 distal
            try:
        ...
        step_allowed: Dict[str, bool] = {}
        for jn in tighten_joints:
            finger = finger_of_joint(jn)
            step_allowed[jn] = (finger == "if") if move_only_if_csv else True

        start_ctrl  = {}
        target_ctrl = {}
        saved_target_ctrl = {}                 # 记录真正的目标，便于“两侧接触后解冻”恢复推进
        ...
        for jn, sgn in zip(tighten_joints, tighten_signs):
            ...
            start_ctrl[jn]  = cur
            tgt = _ctrl_clamp(model, aid, cur + float(sgn) * float(tighten_total_cmd))
            target_ctrl[jn] = tgt
            saved_target_ctrl[jn] = tgt

        # 只让 IF 做 CSV 联动（TH CSV 在只 IF 推进模式下禁用）
        allow_if_csv = (if_prox_name in tighten_joints) and (if_dist_name not in tighten_joints) \
                    and (if_x is not None and if_y is not None) and (if_dist_name in joint2aid)
        allow_th_csv = False if move_only_if_csv else (
            (th_prox_name in tighten_joints) and (th_dist_name not in tighten_joints)
            and (th_x is not None and th_y is not None) and (th_dist_name in joint2aid)
        )

        def one_round(max_steps: int, tag: str):
            """执行一轮向 target_ctrl 逼近；返回 (established: bool)"""
            nonlocal log, target_ctrl
            contacted_if = (not active_if)  # 若该指不参与，则视为“已接触”
            contacted_th = (not active_th)
            both_contacted = False
            consecutive_ok = 0
            steps_taken = 0
            ...
            for s in range(1, max_steps+1):
                ...
                if active_if and (not contacted_if) and (fn_if >= detect_force_eps or d_if <= detect_dist_eps):
                    ...
                    if log["frozen_at"]["if"] < 0:
                        log["frozen_at"]["if"] = len(log["per_step"])
                ...
                if active_th and (not contacted_th) and (fn_th >= detect_force_eps or d_th <= detect_dist_eps):
                    ...
                    for jn in tighten_joints:
                        if finger_of_joint(jn) == "th" and jn in target_ctrl and jn in joint2aid:
                            target_ctrl[jn] = float(data.ctrl[joint2aid[jn]])
                    if log["frozen_at"]["th"] < 0:
                        log["frozen_at"]["th"] = len(log["per_step"])

                # 两侧都接触 → 解冻“允许推进”的一侧（IF），恢复到 saved_target_ctrl
                if (not both_contacted) and (contacted_if and contacted_th):
                    both_contacted = True
                    for jn in tighten_joints:
                        if step_allowed.get(jn, False):
                            target_ctrl[jn] = saved_target_ctrl[jn]   # 允许推进的关节恢复真正目标
                        else:
                            target_ctrl[jn] = float(data.ctrl[joint2aid[jn]])  # 不允许推进的保持冻结
                ...
                # 施加 ctrl 步进
                for jn, sgn in zip(tighten_joints, tighten_signs):
                    if jn not in joint2aid:
                        continue
                    # 仅对允许推进的关节执行步进（move_only_if_csv=True → 只 IF）
                    if not step_allowed.get(jn, True):
                        continue

                    finger = finger_of_joint(jn)
                    # 若某指已接触而另一指未接触：本指保持冻结（不推进）
                    if finger == "if" and contacted_if and (active_th and not contacted_th):
                        ...
                        if abs(step) > 0.0:
                            ...
                            data.ctrl[aid] = newv
                            # IF prox→dist CSV 联动（仅 IF，且 distal 未被选入）
                            ...
                            # TH 的 CSV 在只 IF 推进模式下被禁用（allow_th_csv=False）
        ...
        if (not ok) and extend_enable and (extend_cmd_each > 0.0) and (extend_steps_each > 0) and (extend_max_rounds > 0):
            for r in range(1, extend_max_rounds+1):
                # 只对允许推进的关节追加目标（只 IF）
                for jn, sgn in zip(tighten_joints, tighten_signs):
                    if (jn not in joint2aid) or (not step_allowed.get(jn, True)):
                        continue
                    aid = joint2aid[jn]
                    ...
                    saved_target_ctrl[jn] = max(saved_target_ctrl.get(jn, new_tgt), new_tgt) if (float(sgn) > 0) else \
                                            min(saved_target_ctrl.get(jn, new_tgt), new_tgt)
    ...
    for pair_idx, k in enumerate(eval_ids, start=1):
        ...
        for idx_local, db_idx in enumerate(matching_indices, start=1):
            ...
            target_if = float(cfg.get("tighten_target_force_if", contact_force_eps))
            target_th = float(cfg.get("tighten_target_force_th", contact_force_eps))

            if not (tighten_info.get("established", False) and
                    fn_if1 >= target_if and fn_th1 >= target_th):
                print(f"  → SKIP: no both-finger contact after tighten; "
                    f"if={fn_if1:.3e}, th={fn_th1:.3e}, "
                    f"need_if/th={target_if:.3e}/{target_th:.3e}")
                pair_skip += 1
                continue
